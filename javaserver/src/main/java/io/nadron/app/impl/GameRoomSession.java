...
public abstract class GameRoomSession extends DefaultSession implements GameRoom
{
	 ...
	 */
	protected Map<String,PlayerSession> sessions;
	...
	public static class GameRoomSessionBuilder extends SessionBuilder
	{
		...
		public GameRoomSessionBuilder sessions(Map<String,PlayerSession> sessions)
		{
	...
	public PlayerSession createPlayerSession(Player player)
	{
		...
		if(player != null){
			player.addSession(playerSession);
		}
	...
	public synchronized boolean connectSession(PlayerSession playerSession)
	{
		if (!isShuttingDown)
		{
			...
			sessions.put((String)playerSession.getPlayer().getId(),playerSession);
	...
	public synchronized boolean disconnectSession(PlayerSession playerSession)
	{
		...
		PlayerSession remove = sessions.remove(playerSession.getPlayer().getId());
		//playerSession.getEventDispatcher().clear(); // remove network handlers of the session.
		return (removeHandlers && remove != null);
	}
	...
	{
		isShuttingDown = true;
		Collection<PlayerSession> ps = sessions.values();
		for(PlayerSession session: ps)
		{
			session.close();
		}
	}
