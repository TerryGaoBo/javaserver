...
import com.jelly.player.DefaultPlayerEvent;
...
public class Events
{
	 ...
	 * A network exception will in turn cause this even to be raised.
	 */
	public static final byte GAME_OVER = 0x27;
	...
	public static Event event(ObjectNode source)
	{
		DefaultPlayerEvent event = new DefaultPlayerEvent();
		event.setSource(source);
		event.setType(SESSION_MESSAGE);
		event.setTimeStamp(System.currentTimeMillis());
		return event;
	}
	
	public static Event event(String key,JsonNode source)
	{
		DefaultPlayerEvent event = new DefaultPlayerEvent();
