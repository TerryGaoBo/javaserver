...
import static io.nadron.communication.DeliveryGuaranty.DeliveryGuarantyOptions.INSIDE;
import static io.nadron.communication.DeliveryGuaranty.DeliveryGuarantyOptions.RELIABLE;
...
public class DefaultSessionEventHandler implements SessionEventHandler
{
	...
	protected void onNetworkMessage(NetworkEvent event)
	{
		...
		else if (guaranty.getGuaranty() == RELIABLE.getGuaranty())
		{
			session.getTcpSender().sendMessage(event);
		}else if (guaranty.getGuaranty() == INSIDE.getGuaranty()) {
			//TODO 是否需要区分于外部传过来的消息
			onDataIn(event);
		}
	}
	...
	protected void onException(Event event)
	{
		...
		String reconnectID = (String) session.getAttribute(NadronConfig.RECONNECT_ID);
		...
		if (null != reconnectKey && null != registry)
		{
			...
			if(null == registry.getSession(reconnectKey)){
				...
				//add by zhl
				registry.putUser(reconnectID, reconnectKey);
