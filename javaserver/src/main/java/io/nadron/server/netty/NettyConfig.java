...
public class NettyConfig {
	...
	public synchronized NioEventLoopGroup getBossGroup() {
		if (null == bossGroup) {
			if (0 >= bossThreadCount) {
				...
				bossGroup = new NioEventLoopGroup(bossThreadCount,new NamedThreadFactory("Server-Boss"));
			}
		}
		return bossGroup;
	}
	...
	public synchronized NioEventLoopGroup getWorkerGroup() {
		if (null == workerGroup) {
			if (0 >= workerThreadCount) {
				...
				workerGroup = new NioEventLoopGroup(workerThreadCount,new NamedThreadFactory("Server-Worker"));
			}
		}
		return workerGroup;
	}
	...
	public synchronized InetSocketAddress getSocketAddress() {
		if (null == socketAddress) {
			socketAddress = new InetSocketAddress(getPortNumber());
		}
	...
	public int getPortNumber() {
		if (ContextConfig.isOfficalEnv()) {
			return ContextConfig.getServerPort();
		}
		return portNumber;
	}
